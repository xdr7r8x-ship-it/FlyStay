 ...
 * NO MOCK DATA - Returns real results from Packages provider only.
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await packagesProvider.search();

    if (!result.success) {
      return NextResponse.json({ error: result.error }, { status: 400 });
    }

    return NextResponse.json({ data: result.data, count: (result.data as unknown[])?.length || 0 });
  } catch (error) {
