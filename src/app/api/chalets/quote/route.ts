/**
 * Chalets Quote API
 * POST /api/chalets/quote
 * 
 * FAIL-CLOSED: Returns 503 if internal inventory not enabled.
 * NO MOCK DATA - Returns real quote from internal inventory only.
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await internalInventoryProvider.quote(validatedInput);
    ...
    console.error('Chalets quote error:', error);
    return NextResponse.json(
